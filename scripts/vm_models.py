    ...
    def set_interfaces(self, config):
        ...
        for interface in self.interfaces:
            ...
            if interface['host_only']:
                ...
                ip: \"%s\", netmask: \"%s\", nic_type: \'82540EM\'""" % (
            ...
            inline: \"/bin/sh /tmp/config-%s.sh\"""" % (
              os.path.join(ansible_scripts_path, "set_interface.yml"),
              str("ifcfg-eth"+str(ifcount)), interface['ip'],
              interface['netmask'], str("ifcfg-eth"+str(ifcount)))
            ifcount += 1
        return config
