...
class LabParser():
    ...
    def create(cls, args):
        ...
        if input_params.get('template') not in cls.topo_dict.keys():
            print(Fore.RED + "Note: " + Fore.WHITE +
              "Invalid template. Template should be one in {}".format(cls.topo_dict.keys()))
            sys.exit()
        ...
        for name, item in info.items():
            ...
            if 'flavour' in item.keys():
                for node in item['flavour']:
                    ...
                    total_memory = total_memory + cls.mb_to_gb(vm.flavour[flavour]['memory'])
            else:
                total_memory = None
    ...
    def destroy(cls, args):
        ...
        if topo_info['host_vboxnet_ip'] != []:
            for honly_interface in topo_info['host_vboxnet_ip']:
                ...
                vboxnet_ip = re.findall(r'Name:\s+(vboxnet\d)[\s\S]{{1,100}}IPAddress:\s+{}'.format(
                             honly_interface), op)
                if vboxnet_ip == []:
