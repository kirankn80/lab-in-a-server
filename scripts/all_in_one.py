...
class AllInOne(BasicTopology):
    def __init__(self, input_params):
        ...
        super().__init__("all_in_one", input_params, self.total_nodes, self.contrail_version)
    ...
    def get_contrail_deployer_branch(self):
        ...
        branches_info = requests.get(contrail_list_branch_api).json() + requests.get(tf_list_branch_api).json()
    ...
    def provision_contrail(self):
        if self.contrail_version:
            ...
            AllInOneContrail().provision(self.hosts[0], self.contrail_version,
    ...
    def show(cls, topo_info):
        ...
        if instances_file_path != "DOES NOT EXIST":
            ...
            table.add_row(
                ["Contrail Version", contrail_info['contrail_configuration']['CONTRAIL_VERSION']])
            table.add_row(["Cloud Orchestrator",
            ...
            if 'kolla_internal_vip_address' in contrail_info['kolla_config']['kolla_globals'].keys():
                table.add_row(["kolla_internal_vip_address", contrail_info['kolla_config']
                     ['kolla_globals']['kolla_internal_vip_address']])
            if 'kolla_external_vip_address' in contrail_info['kolla_config']['kolla_globals'].keys():
                table.add_row(["kolla_external_vip_address", contrail_info['kolla_config']
            ...
            if 'contrail_api_interface_address' in contrail_info['kolla_config']['kolla_globals'].keys():
                table.add_row(["contrail_api_interface_address", contrail_info['kolla_config']
        ...
        for host in topo_info['hosts']:
            ...
            if host not in topo_info['management_data'].keys() or topo_info['management_data'][host] == {}:
                ...
                table.add_row(
                        ["default gateway", topo_info['management_data'][host]['gateway']])
